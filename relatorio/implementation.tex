...
Após obter os diretórios todos é importante conseguir trabalhar sobre cada um deles, para isso usamos o \mintinline{bash}{while read} da seguinte maneira:
...
sendo que diretórios podem conte-los e não queremos cortá-los. O \mintinline{bash}{-d $'\0'} tem um
...
tamanho através de outro \mintinline{bash}{find}, contudo desta vez ele terá
algumas diferenças.
...
ficheiros em bytes. O argumento \mintinline{bash}{--files0-from=-} é necessário aqui visto que serve para ler caminhos do \Verb|stdin| separados por \Verb|'\0'|, que é como nós formatámos os dados no passo anterior com o
\mintinline{bash}{-print0} no \mintinline{bash}{find}. Por fim, usamos a opção
\mintinline{bash}{-cs} na qual o \Verb|c| mostra a soma dos tamanhos de todos os ficheiros e o \Verb|s| serve para evitar repetições
(o seu único propósito é eficiência).

De seguida usamos o comando \mintinline{bash}{cut -f1} para obter tudo até o
primeiro \Verb|'\t'| e, por fim, damos \mintinline{bash}{tail -n1} para extrair
só a última linha, que é a que contém o valor total.

Se ocorrer algum erro no \mintinline{bash}{du} o tamanho do diretório será substituido por \Verb|NA|.
...
A opção \Verb|-newermt| faz com que o \Verb|find| apenas procure por ficheiros com data de modificação mais recente ou igual à data passada. Esta é convertida para unix time através do comando \mintinline{bash}{date -d "$data" +%s}, e é assim que é passada ao \Verb|find|.
...
Após isso são feitos dois \mintinline{bash}{while read} um para cada ficheiro passado como argumento ao script.
